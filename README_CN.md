...
`rsws` **完全符合** [RFC 6455 - WebSocket 协议](https://tools.ietf.org/html/rfc6455) 规范。
...
|------|------|------|
| §4 | 开启握手 | ✅ 完全支持 |
| §5.2 | 帧格式（FIN、RSV、Opcode、Mask、Payload） | ✅ 完全支持 |
| §5.3 | 客户端到服务端掩码 | ✅ 完全支持 |
| §5.4 | 消息分片 | ✅ 完全支持 |
| §5.5 | 控制帧（Close、Ping、Pong） | ✅ 完全支持 |
| §6 | UTF-8 验证 | ✅ 完全支持 |
| §7 | 关闭握手 | ✅ 完全支持 |
| §7.4 | 状态码（1000-4999） | ✅ 完全支持 |
| §9 | 扩展 | ✅ 完全支持 |
| §10 | 安全性（Origin、CSWSH） | ✅ 完全支持 |

### 已实现功能

- **帧解析与序列化**：所有操作码（0x0-0xA）、7/16/64 位负载长度
- **掩码处理**：SIMD 加速 XOR（AVX2/SSE2/NEON），吞吐量 >150 GiB/s
- **消息分片**：支持可配置限制的消息重组
- **控制帧**：强制执行负载 ≤125 字节、禁止分片
- **关闭码**：所有标准码（1000-1015）及应用程序自定义码（3000-4999）
- **UTF-8 验证**：跨分片增量验证
- **安全加固**：
  - Origin 验证（CSWSH 防护）
  - CRLF 注入防护
...
| 扩展 | RFC | 状态 |
|------|-----|------|
| permessage-deflate | [RFC 7692](https://tools.ietf.org/html/rfc7692) | ✅ 已实现（功能门控） |
